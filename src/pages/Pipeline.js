  ...
  async function confirmar() {
    ...
    if (gerarContrato) {
      ...
      const res = await saveContrato({
        ...
        vigencia_fim:    '',
        itens_adesao:  (opp.itens||[]).filter(i=> i.tipo==='licenca' || (i.cobranca==='unico' && !['servico','consultoria'].includes(i.tipo))).map(i=>({ produto_id: i.produto_id, nome: i.produto_nome||i.nome||'', valor: i.subtotal||i.valor||0, tabela: i.preco_unitario||i.valor||null, desconto_pct: i.desconto_pct||0, desconto_autorizado: false })),
        itens_mrr:     (opp.itens||[]).filter(i=> !['servico','consultoria','licenca'].includes(i.tipo) && ['mensal','anual','uso','usuario'].includes(i.cobranca)).map(i=>({ produto_id: i.produto_id, nome: i.produto_nome||i.nome||'', valor: i.subtotal||i.valor||0, tabela: i.preco_unitario||i.valor||null, desconto_pct: i.desconto_pct||0, desconto_autorizado: false })),
        itens_servico: [],
        responsavel:         opp.responsavel || '',
