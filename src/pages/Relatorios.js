...
import { DATA_SOURCES, COMPONENT_TYPES, PRESET_CORES } from '../data/mockRelatorios'

// ── Agregação ──────────────────────────────────────────────────────────────

// ── Join em memória (left join) ────────────────────────────────────────────
...
// Detecta campo "empresa" em qualquer fonte (empresa, empresa_nome, company_nome…)
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width:'100%', height:altura }}>
      {Array.from({ length: yLines + 1 }, (_, i) => {
        const y = PAD.top + (plotH / yLines) * i
        const val = maxVal * (1 - i / yLines)
        return (
          <g key={i}>
            <line x1={PAD.left} y1={y} x2={W - PAD.right} y2={y}
              stroke="var(--border)" strokeWidth="1" />
            <text x={PAD.left - 6} y={y + 4} textAnchor="end" fontSize="10"
              fill="var(--text-muted)">{fmtNum(val)}</text>
          </g>
        )
      })}
      {dados.map((d, i) => {
        ...
        const isDrilled = drillLabel === d.label
        return (
          <g key={i}
            onMouseEnter={() => setHover(i)}
            onMouseLeave={() => setHover(null)}
            onClick={() => onDrill?.(d.label)}
            style={{ cursor: onDrill ? 'pointer' : 'default' }}>
            <rect x={x} y={y} width={barW} height={barH}
              fill={cor}
              opacity={isDrilled ? 1 : isHover ? 0.95 : 0.82}
              rx="3"
              stroke={isDrilled ? '#fff' : 'none'}
              strokeWidth={isDrilled ? 2 : 0} />
            <text x={x + barW / 2} y={y - 6} textAnchor="middle" fontSize="10"
              fill="var(--text-soft)" fontWeight={isHover || isDrilled ? '700' : '400'}>
              {fmtNum(d.valor)}
            </text>
            <text
              x={x + barW / 2} y={PAD.top + plotH + 14}
              textAnchor="end" fontSize="10" fill={isDrilled ? cor : 'var(--text-muted)'}
              fontWeight={isDrilled ? '700' : '400'}
              transform={`rotate(-35, ${x + barW / 2}, ${PAD.top + plotH + 14})`}>
              {String(d.label).slice(0, 18)}
            </text>
            {isHover && (
              <g>
                <rect x={x + barW / 2 - 52} y={y - 34} width={104} height={22}
                  rx="4" fill="var(--surface)" stroke="var(--border)" />
                <text x={x + barW / 2} y={y - 19} textAnchor="middle" fontSize="11"
                  fill="var(--text)" fontWeight="600">
                  {fmtNum(d.valor)}{onDrill ? ' · clique p/ detalhar' : ''}
                </text>
              </g>
            )}
          </g>
...
function PieChart({ dados, onDrill, drillLabel }) {
  ...
  const fatias = dados.map((d, i) => {
    ...
    return { ...d, frac, x1, y1, x2, y2, large, cor: PRESET_CORES[i % PRESET_CORES.length] }
  })
  return (
      ...
      <svg viewBox="0 0 200 200" style={{ width:180, height:180, flexShrink:0 }}>
        {fatias.map((f, i) => (
          <path key={i}
            ...
            fill={f.cor}
            opacity={drillLabel === f.label ? 1 : hover === i ? 1 : 0.82}
        ...
        {fatias.map((f, i) => (
          <div key={i}
            ...
            style={{ display:'flex', alignItems:'center', gap:8,
              ...
              fontWeight: drillLabel === f.label ? 700 : 400 }}>
...
function LineChart({ dados, cor, altura = 220 }) {
  ...
  const pts = dados.map((d, i) => ({
    ...
    y: PAD.top  + plotH - (d.valor / maxVal) * plotH,
    ...d,
  }))
  const polyline = pts.map(p => `${p.x},${p.y}`).join(' ')
  const area = `M${pts[0].x},${PAD.top + plotH} ` +
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width:'100%', height:altura }}>
      ...
      <polyline points={polyline} fill="none" stroke={cor} strokeWidth="2.5"
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center',
      ...
      <div style={{ fontSize:48, fontWeight:800, color: cor || 'var(--accent)',
        fontFamily:'var(--mono)', lineHeight:1, textAlign:'center' }}>
        {fmtNum(valor)}
      </div>
      {subtitulo && (
        <div style={{ fontSize:12, color:'var(--text-muted)', textAlign:'center' }}>{subtitulo}</div>
      )}
    </div>
  )
}
...
function DrillPanel({ drill, onClose }) {
  // drill = { sourceId, campo, valor, campoX, registros }
  ...
  const linhas = registros.filter(r =>
    String(r[campo] ?? '').toLowerCase() === String(valor).toLowerCase()
  )
  return (
    <div style={{
      position:'absolute', bottom:0, left:260, right:0,
      background:'var(--surface)', borderTop:'2px solid var(--accent)',
      boxShadow:'0 -4px 24px #0002', zIndex:50, display:'flex', flexDirection:'column',
      maxHeight:260,
    }}>
...
// ── Estilos compartilhados para ConfigPanel ────────────────────────────────
...
const cfgSelect = {
  ...
  fontFamily:'var(--font)', boxSizing:'border-box', marginTop:4,
}

// ── Painel de configuração direito ─────────────────────────────────────────

function ConfigPanel({ widget, source, onUpdate, onClose }) {
  ...
  return (
    <div style={{ width:280, flexShrink:0, background:'var(--surface)',
      ...
      <div style={{ padding:'12px 16px', borderBottom:'1px solid var(--border)',
        ...
        <button onClick={onClose}
          style={{ background:'none', border:'none', color:'var(--text-muted)', cursor:'pointer', fontSize:16, padding:'2px 4px' }}>
          ✕
        </button>
      </div>
      ...
      <div style={{ padding:'16px', display:'flex', flexDirection:'column', gap:14 }}>
        ...
        <div>
          ...
          <select style={cfgSelect} value={widget.sourceId || ''}
            ...
            {DATA_SOURCES.map(s => <option key={s.id} value={s.id}>{s.icon} {s.label}</option>)}
          </select>
        </div>
        ...
        <div>
          ...
          <select style={cfgSelect} value={widget.joinSourceId || ''}
            ...
            {DATA_SOURCES.filter(s => s.id !== widget.sourceId).map(s =>
              <option key={s.id} value={s.id}>{s.icon} {s.label}</option>
            )}
          </select>
          {widget.joinSourceId && (() => {
            const joinSrc = DATA_SOURCES.find(s => s.id === widget.joinSourceId)
            return (
        ...
        {widget.tipo !== 'kpi' && widget.tipo !== 'table' && (
          <div>
            ...
            <select style={cfgSelect} value={widget.campoX || ''}
              onChange={e => onUpdate({ campoX: e.target.value })}>
              ...
              {campos.map(f => {
                ...
                return (
                  <label key={f.key} style={{ display:'flex', alignItems:'center', gap:8,
                    fontSize:12, color:'var(--text-soft)', cursor:'pointer' }}>
        ...
        {widget.tipo !== 'table' && (
          <div>
            ...
            <div style={{ display:'flex', gap:6, marginTop:6 }}>
              <select style={{ ...cfgSelect, flex:'0 0 90px' }}
                value={widget.metrica || 'COUNT'}
                onChange={e => onUpdate({ metrica: e.target.value })}>
                {['COUNT','SUM','AVG','MAX','MIN'].map(m => <option key={m}>{m}</option>)}
              </select>
              {widget.metrica !== 'COUNT' && (
                <select style={{ ...cfgSelect, flex:1 }} value={widget.campoY || ''}
                  onChange={e => onUpdate({ campoY: e.target.value })}>
                  <option value="">— campo —</option>
        ...
        <div>
          ...
          <div style={{ fontSize:10, color:'var(--text-muted)', marginBottom:4 }}>
            Ex: <code style={{ fontFamily:'var(--mono)', background:'var(--surface2)',
              padding:'1px 4px', borderRadius:3 }}>SUM(valor) / COUNT(*) * 100</code>
          </div>
          ...
          {widget.formula && source && (() => {
            ...
            return (
              <div style={{ marginTop:6, fontSize:12,
                color: res !== null ? 'var(--accent)' : '#EF4444',
            ...
            {PRESET_CORES.map(c => (
              <button key={c} onClick={() => onUpdate({ cor: c })}
                style={{ width:24, height:24, borderRadius:6, background:c,
                  border:`2.5px solid ${widget.cor === c ? 'var(--text)' : 'transparent'}`,
                  cursor:'pointer' }} />
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          {(widget.filtros || []).map((f, i) => (
            <div key={i} style={{ display:'flex', gap:4, marginTop:4 }}>
              <select style={{ ...cfgSelect, flex:1 }} value={f.campo || ''}
                onChange={e => {
                  const fs = [...(widget.filtros || [])]; fs[i] = { ...f, campo: e.target.value }; onUpdate({ filtros: fs })
                }}>
                <option value="">campo</option>
              ...
              <input style={{ ...cfgSelect, flex:1 }} placeholder="valor" value={f.valor || ''}
                onChange={e => {
                  const fs = [...(widget.filtros || [])]; fs[i] = { ...f, valor: e.target.value }; onUpdate({ filtros: fs })
                }} />
          ...
          <button onClick={() => onUpdate({ filtros: [...(widget.filtros || []), { campo:'', valor:'' }] })}
            style={{ marginTop:6, fontSize:11, color:'var(--accent)', background:'none', border:'none',
...
function renderWidget(w, source, { filtrosGlobais = {}, onDrill, drillState } = {}) {
  let regs = source?.registros || []

  // 1. Join com fonte secundária
  if (w.joinSourceId && w.joinLeft && w.joinRight) {
    const joinSrc = DATA_SOURCES.find(s => s.id === w.joinSourceId)
  ...
  if (filtrosGlobais.empresa) {
    ...
    if (cf) regs = regs.filter(r =>
      String(r[cf] || '').toLowerCase().includes(filtrosGlobais.empresa.toLowerCase())
    )
  }
  ...
  ;(w.filtros || []).forEach(f => {
    if (f.campo && f.valor) {
      regs = regs.filter(r => String(r[f.campo] || '').toLowerCase().includes(f.valor.toLowerCase()))
    }
  })
  ...
  if (w.tipo === 'table') {
    // se há drill ativo nesta fonte, filtra a tabela
    let linhas = regs
    if (drillState && drillState.sourceId === w.sourceId && drillState.campo) {
      linhas = regs.filter(r =>
        String(r[drillState.campo] ?? '').toLowerCase() === String(drillState.valor).toLowerCase()
      )
    }
...
export default function Relatorios() {
  const [nomeRelatorio, setNomeRelatorio] = useState('Novo relatório')
  ...
  const [drillState, setDrillState]         = useState(null)  // { sourceId, campo, valor, registros }

  // Coleta todos os valores únicos de "empresa" das fontes usadas nos widgets
  const empresasDisponiveis = Array.from(new Set(
    widgets.flatMap(w => {
      const src = DATA_SOURCES.find(s => s.id === w.sourceId)
      if (!src) return []
  ...
  const sourceAtual = selecionado ? DATA_SOURCES.find(s => s.id === selecionado.sourceId) : null
  ...
  function exportarCSV() {
    ...
    const src = DATA_SOURCES.find(s => s.id === selecionado.sourceId)
    if (!src) return
    const campos = src.fields
  ...
  return (
    <div style={{ display:'flex', height:'100vh', overflow:'hidden',
      ...
      <div style={{ width:260, flexShrink:0, background:'var(--surface)',
        ...
        <div style={{ flex:1, overflowY:'auto', padding:'12px 10px' }}>
          ...
          {abaLateral === 'fontes' && DATA_SOURCES.map(src => {
                  ...
                  <div style={{ paddingLeft:28, paddingBottom:4 }}>
                    {src.fields.map(f => (
                      <div key={f.key} style={{ display:'flex', alignItems:'center', gap:6,
                        padding:'4px 8px', borderRadius:5, fontSize:12, color:'var(--text-muted)' }}>
                        <span style={{ fontSize:9, color:'var(--text-muted)', background:'var(--surface2)',
              ...
              {COMPONENT_TYPES.map(t => (
                <button key={t.id} onClick={() => addWidget(t.id)}
                  style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:6,
                    ...
                    background:'var(--surface2)', cursor:'pointer', fontFamily:'var(--font)',
                    transition:'border-color 0.15s, box-shadow 0.15s' }}
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', overflow:'hidden' }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 20px',
          ...
          {editandoNome ? (
            <input autoFocus value={nomeRelatorio}
              ...
              style={{ fontSize:15, fontWeight:700, color:'var(--text)', background:'none',
                border:'none', outline:'none', borderBottom:'2px solid var(--accent)',
                fontFamily:'var(--font)', minWidth:200 }} />
          ) : (
          ...
          <button onClick={() => alert('Relatório salvo!')}
            style={{ padding:'7px 18px', background:'var(--accent)', color:'#fff',
          ...
          <div style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 20px',
            ...
            {(filtrosGlobais.periodo !== 'todos' || filtrosGlobais.empresa || drillState) && (
              <div style={{ display:'flex', gap:6, alignItems:'center', flex:1 }}>
                {filtrosGlobais.periodo !== 'todos' && (
                  <span style={{ fontSize:11, padding:'2px 8px', background:'var(--accent)18',
                    color:'var(--accent)', borderRadius:20, border:'1px solid var(--accent)44' }}>
                    {({ este_mes:'Este mês', ultimos_3m:'Últ. 3 meses', este_ano:'Este ano' })[filtrosGlobais.periodo]}
                  </span>
                )}
                {filtrosGlobais.empresa && (
                  <span style={{ fontSize:11, padding:'2px 8px', background:'var(--accent)18',
                    color:'var(--accent)', borderRadius:20, border:'1px solid var(--accent)44' }}>
                    {filtrosGlobais.empresa}
                  </span>
                ...
                {drillState && (
                  <span style={{ fontSize:11, padding:'2px 8px', background:'#F59E0B18',
                    color:'#D97706', borderRadius:20, border:'1px solid #F59E0B44', cursor:'pointer' }}
                    onClick={() => setDrillState(null)}>
                    drill: {drillState.campo} = {drillState.valor} ✕
                  </span>
                )}
                <button onClick={() => { setFiltrosGlobais({ empresa:'', periodo:'todos' }); setDrillState(null) }}
                  style={{ fontSize:11, color:'var(--text-muted)', background:'none', border:'none',
                    cursor:'pointer', padding:'2px 4px', fontFamily:'var(--font)' }}>
                  Limpar
                </button>
              </div>
            )}
        ...
        {/* grid de widgets — posição relativa para DrillPanel absoluto */}
          ...
          {widgets.length === 0 ? (
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center',
              ...
              <div style={{ fontSize:15, fontWeight:600, color:'var(--text-soft)' }}>
                Comece adicionando um visual
              </div>
              <div style={{ fontSize:13, color:'var(--text-muted)' }}>
                Escolha um componente no painel esquerdo ou clique abaixo
              </div>
            ...
            <>
              <div
                style={{ display:'grid', gridTemplateColumns:'repeat(12, 1fr)', gap:16, alignItems:'start' }}
              ...
              {/* DrillPanel — sobrepõe a grade pela base */}
              {drillState && (
                <DrillPanel drill={drillState} onClose={() => setDrillState(null)} />
              )}
            </>
          )}
        </div>
