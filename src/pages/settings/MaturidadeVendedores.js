...
import { useSellerMaturity, ORIGENS, CONDICOES } from '../../hooks/useSellerMaturity'
...
export default function MaturidadeVendedores() {
  const { params, loading, save, remove } = useSellerMaturity()
  ...
  if (slideOpen) {
    return (
      <FullPageEdit
        ...
        breadcrumb={[{ label: 'Maturidade de Vendedores', onClick: () => { setSlideOpen(false); setEditando(null) } }]}
        ...
        <FPESection title="Identificação">
          <FPEField label="Nome *" error={errs.nome} span={2}>
            <input
              ...
              placeholder="Ex: Oportunidades ganhas no trimestre"
              style={inputS}
            />
          </FPEField>
  ...
  return (
    <SettingsLayout
      modulo="maturidade_vendedores"
      title="Maturidade de Vendedores"
      description={`Configure os parâmetros que definem o score de maturidade de cada Contato Canal.${totalPeso > 0 ? `  Peso total ativo: ${totalPeso}pts` : ''}`}
      columns={columns}
      data={filtered}
      keyField="id"
      loading={loading}
      ...
      emptyLabel="Nenhum parâmetro configurado. Crie parâmetros para calcular a maturidade dos Contatos Canais."
    />
  )
}
